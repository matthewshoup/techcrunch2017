(function($) {
    $(function() {
        var videoProfile = Cookies.get("videoProfile") || "480p_4",
            // maxFrameRate = Number(Cookies.get("maxFrameRate") || 30),
            // maxBitRate = Number(Cookies.get("maxBitRate") || 1000),
            channel = Cookies.get("roomName"),
            key = "154b1a9a660d4a35afcdae14a3756255",
            remoteStreamList = [],
        ...
        } else if(mode == 'interop-communication') {
                ...
                for (index = 0, length = remoteStreamList.length; index < length; index += 1) {
                    ...
                    if (obj.id === streamId) {
                        if (obj.audioEnabled) {
                            ...
                            $(e.target).attr("src", "assets/images/icon_speak.png");
                        } else {
                            obj.stream.enableAudio();
                            obj.audioEnabled = true;
                            $(e.target).attr("src", "assets/images/icon_mute.png");
                        }
                    }
                }
            });
        ...
        function calculateVideoSize(multiple) {
            var viewportWidth = $(window).width(),
                ...
                curResolution = getResolutionArray(videoProfile),
                width,
                height,
                newWidth,
                newHeight,
        ...
        function subscribeMouseHoverEvents() {
            $(".mute-button").off("hover").hover(function(e) {
                if (disableAudio) {
                    $(e.target).attr("src", "assets/images/btn_mute.png");
                } else {
                    $(e.target).attr("src", "assets/images/btn_mute_touch.png");
                }
            }, function(e) {
                if (disableAudio) {
                    $(e.target).attr("src", "assets/images/btn_mute_touch.png");
            ...
            $(".switch-audio-button").off("hover").hover(function(e) {
                if (disableVideo) {
                    $(e.target).attr("src", "assets/images/btn_video_touch.png");
                } else {
                    $(e.target).attr("src", "assets/images/btn_voice_touch.png");
                }
            }, function(e) {
                if (disableVideo) {
                    $(e.target).attr("src", "assets/images/btn_video.png");
                } else {
                    $(e.target).attr("src", "assets/images/btn_voice.png");
                }
            });

            $(".fullscreen-button").off("hover").hover(function(e) {
                if (fullscreenEnabled) {
                    $(e.target).attr("src", "assets/images/btn_reduction_touch.png");
                } else {
                    $(e.target).attr("src", "assets/images/btn_maximize_touch.png");
                }
            }, function(e) {
                if (screenfull.isFullscreen) {
                    $(e.target).attr("src", "assets/images/btn_reduction.png");
                } else {
                    $(e.target).attr("src", "assets/images/btn_maximize.png");
                }
            });

            $(".expension-button").off("hover").hover(function(e) {
                if (hideLocalStream) {
                    $(e.target).attr("src", "assets/images/btn_expansion.png");
                } else {
                    $(e.target).attr("src", "assets/images/btn_expansion_touch.png");
                }
            }, function(e) {
                if (hideLocalStream) {
                    $(e.target).attr("src", "assets/images/btn_expansion_touch.png");
            ...
            $(".switch-audio-button").off("click").click(function(e) {
                ...
                if (disableVideo) {
                    ...
                    $("#player_" + localStream.getId()).css({
                        ...
                        "background-image": "url(assets/images/icon_default.png)",
